...
class Voicemail(db.Model):
    ...
    # Idempotency protection (prevents duplicate ingestion)
    s3_key = db.Column(db.String(500), unique=True, nullable=True)

    # Caller metadata extracted from email headers
    caller_phone = db.Column(db.String(30), nullable=True)
    ...
    # --------------------
    # Patient Information (AI Extracted)
    # --------------------

    patient_name = db.Column(db.String(255), nullable=True)
    patient_phone = db.Column(db.String(30), nullable=True)
    patient_dob = db.Column(db.String(50), nullable=True)
    call_reason = db.Column(db.Text, nullable=True)

    # AI action routing fields
